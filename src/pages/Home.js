...
const handleUpload = async () => {
    ...
    try {
        ...
        setOriginalData(adjustedData); // Salva o estado original dos dados
...
// Carregar os valores de Ano - Início e Ano - Fim salvos no localStorage
useEffect(() => {
    // Carregar anos do filtro salvos no localStorage
    ...
    // Carregar `data` e `savedData` do localStorage
...
const handleResetAll = () => {
    // Limpa todos os estados relevantes
    setFile(null);
    setNomePessoa('');
    setSavedData([]);
    setEditableData([]);
  ...
  const handleSaveEdit = () => {
    ...
    localStorage.setItem('savedDataPrincipal', JSON.stringify(data)); // Salva `data` principal no localStorage

    // Salva os anos de filtro no localStorage
  ...
  const handleResetEdit = () => {
    if (originalData.length > 0) {
        setEditableData([...originalData]); // Restaura editableData para o estado original do upload
        setFilteredData([...originalData]); // Restaura filteredData para o estado original do upload
        recalcularTabela2(originalData); // Recalcula pontuação com os dados originais

        // Restaura os anos de filtro do localStorage, se necessário
        const savedAnoInicio = localStorage.getItem('savedAnoInicio');
        const savedAnoFim = localStorage.getItem('savedAnoFim');
        setAnos({
            anoInicio: savedAnoInicio || '',
        ...
        <div className="action-buttons">
          ...
          >
			...
			<Button
			  variant="contained"
			  color="secondary"
			  onClick={handleResetAll} // Função que criamos para resetar tudo
