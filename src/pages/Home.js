...
const generatePDF = () => {
  ...
  // Usar html2canvas com ajuste de escala para melhorar a qualidade
  html2canvas(input, { scale: 2 }).then((canvas) => {
    const imgData = canvas.toDataURL('image/jpeg', 0.9); // Usar JPEG com qualidade de 0.9 para um bom equilíbrio entre qualidade e tamanho
    const pdf = new jsPDF('p', 'mm', 'a4');
    const pageWidth = pdf.internal.pageSize.getWidth();
    const pageHeight = pdf.internal.pageSize.getHeight();
    const margin = 10; // Margem de 10 mm em cada lado
    const imgWidth = pageWidth - margin * 2; // A4 width in mm minus margins
    const imgHeight = (canvas.height * imgWidth) / canvas.width;
    let position = margin; // Começar a desenhar a imagem após a margem

    // Verificar se a altura da imagem ultrapassa a altura da página (considerando margens)
    if (imgHeight > pageHeight - margin * 2) {
      let remainingHeight = imgHeight;

      while (remainingHeight > 0) {
        pdf.addImage(imgData, 'JPEG', margin, position, imgWidth, Math.min(remainingHeight, pageHeight - margin * 2));
        remainingHeight -= pageHeight - margin * 2;
        if (remainingHeight > 0) {
          pdf.addPage();
          position = margin; // Resetar a posição para a nova página
        }
      }
    } else {
    ...
    // Nome do arquivo baseado no nome do docente
    ...
    <Container className="container-ppgec" maxWidth={false} style={{ paddingLeft: 70, paddingRight: 70, maxWidth: '1600px', margin: '0 auto' }}>
	 ...
	 <div className="logo-container">
      ...
      <div className="anos-container">
        <TextField label="Ano - Início" name="anoInicio" type="number" value={anos.anoInicio} onChange={handleAnoChange} variant="outlined" style={{ fontSize: 12, fontWeight: 'bold', height: 60, width: 100, textAlign: 'center' }} />
        <TextField label="Ano - Fim" name="anoFim" type="number" value={anos.anoFim} onChange={handleAnoChange} variant="outlined" style={{ fontSize: 12, fontWeight: 'bold', height: 60, width: 100, textAlign: 'center' }} />
        <TextField label="Ponderação Discente (%)" type="number" value={ponderacaoDiscente} onChange={handlePonderacaoChange} variant="outlined" style={{ fontSize: 12, fontWeight: 'bold', height: 60, width: 170, textAlign: 'center' }} />
...
<div className="tabela2-container">
    ...
    <Table className="table-qualis">
      ...
      <TableBody>
        {pontuacaoQualis.map((row, index) => {
          ...
          const totalB = b1 + b2 + b3 + b4; // Calcula o total de B
          const pqd1_ab = totalA + totalB;
          const pqd1 = pqd1_ab + (0.20 * totalA);
          ...
          return (
            <TableRow key={index} className={index % 2 === 0 ? 'even-row' : 'odd-row'}>
              ...
              <TableCell className="col-a1">{a1.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-a2">{a2.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-a3">{a3.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-a4">{a4.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-b1">{b1.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-b2">{b2.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-b3">{b3.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-b4">{b4.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-c">{(row.C || 0).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</TableCell>
              <TableCell className="col-np">{(row.NP || 0).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</TableCell>
              <TableCell className="col-percent-b">{percentB.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}</TableCell>
              <TableCell className="col-total-b">{totalB.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-percent-a">{percentA.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}</TableCell>
              <TableCell className="col-pqd1-a">{totalA.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-pqd1">{pqd1.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
              <TableCell className="col-pqd1-ab">{pqd1_ab.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
            </TableRow>
          );
        })}
      </TableBody>
  ...
  {/* Caixa de Pontuação ao lado direito da tabela */}
  <div className="pontuacao-container">
    <h3>Pontuação</h3>
    <p className="pontuacao-value">
      {pontuacaoQualis.reduce((acc, row) => acc + (row.TotalA || 0) + (row.TotalB || 0), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
    </p>
    
    {/* Botão Gerar PDF abaixo da caixa de pontuação */}
